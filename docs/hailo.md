...
```bash
mkdir -p ~/photage/models
```

Set `PHOTAGE_MODELS_PATH=/home/<you>/photage/models` in `.env`, then:
...
installed runtime, downloads to `PHOTAGE_MODELS_PATH`, **verifies the SDK version
...
Then plain `docker compose up -d` and `docker compose logs -f photage` do the
right thing.
...
```bash
docker compose exec photage /usr/bin/python3 -c \
...
```bash
docker compose exec photage ls /opt/hailo/python/hailo_platform
docker compose exec photage printenv PYTHONPATH
```
...
```bash
docker compose exec photage ls -l /opt/hailo/lib/
docker compose exec photage printenv LD_LIBRARY_PATH
```
...
```bash
/usr/bin/python3 -V
docker compose exec photage /usr/bin/python3 -V
```
...
```
PHOTAGE_HAILO_API=infer_model     # Hailo-10H
PHOTAGE_HAILO_API=vstreams        # Hailo-8
```
...
```bash
docker compose logs photage | grep -iE "VLM HEF|falling back"
```
...
must be in `PHOTAGE_MODELS_PATH`, named exactly `qwen2-vl-2b-instruct.hef`.
...
```bash
docker compose exec photage ls -l /app_cache/models
```
...
docker compose exec photage ls -la /app/snex/projects/*/

# is uv there?
docker compose exec photage uv --version
```
...
```bash
cd ~/photage && docker compose up -d      # no -f overlay
```
