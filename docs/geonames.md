...
docker compose exec photage /app/bin/photage eval 'Photage.Release.load_gazetteer(force: true)'

# load a different dataset
docker compose exec photage /app/bin/photage eval 'Photage.Release.load_gazetteer(dataset: "cities1000")'
```
...
`PHOTAGE_GEONAMES_BASE_URL` is only for the paid endpoint
...
`photage-cache` volume — `docker compose pull && up -d` touches neither.
...
- **Run the diagnostic.** Early versions shipped a shared geonames.org account
  ...
  source above.
- **Refresh your compose files.** `PHOTAGE_LOAD_GAZETTEER` has to be present in
  the `photage` service's `environment:` block to reach the container at all.
...
**`service "web" is not running`.** The service is named `photage`. Older
