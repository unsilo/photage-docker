...
`raw.githubusercontent.com/unsilo/photage-docker/main/...`. The local repo is on
...
Or change `PHOTAGE_REF`/`RAW` in `install.sh` and every raw URL in the docs. The
branch is the cheaper fix.

### 2. Registry name

`deploy/docker/build-and-push.sh` in the photage repo pushes to
**`tehsnappysoftware/photage`**. `deploy/docker/docker-compose.rpi5.yml` in that
same repo still pulls **`tehsnappysoftware/photage`** — an underscore, a
different repository, and nothing would ever pull.

This repo standardises on `tehsnappysoftware/photage`. Delete or fix the stale
compose files (see §7).
...
waiting at `.github/workflows/publish.yml` in the *photage* repo — see §3a. When
...
`~/projects/photage/.github/workflows/publish.yml`. Builds each architecture on
...
- [ ] `docker compose ps` shows all three services, `photage` healthy
- [ ] `http://<host>` loads the login page
- [ ] the seeded admin can log in
- [ ] a photo dropped in `~/photage/import` imports
- [ ] its thumbnail renders (this is the `PHOTAGE_IMAGE_URL_BASE` check)
...
- [ ] create `tehsnappysoftware/photage`, public
- [ ] short description: "Self-hosted photo archive for Raspberry Pi and Linux"
- [ ] full description: point at
      `https://github.com/unsilo/photage-docker` and paste the quick start
...
### 7. Retire the superseded files in the photage repo
...
`github.com/unsilo/photage-docker` and noting that `Dockerfile.web` and
...
```bash
cd ~/projects/photage
git tag -a v0.1.0 -m "Photage 0.1.0"
git push --tags
```
...
```bash
docker run --rm --entrypoint sh photage:test -c \
  'ls /app/lib/photage-0.1.0/ebin | head'
```
...
```bash
docker history --no-trunc photage:test | grep -iE 'key|secret|password|token'
dive photage:test          # or: docker save photage:test | tar -tv | less
```
...
| Claim | Where | Status |
|---|---|---|
| Hailo overlay works | `docs/hailo.md`, `docker-compose.hailo.yml` | **Untested.** Marked experimental everywhere it appears. The host-mount approach is sound in principle; the Python-ABI constraint is real and documented. Verify on `photage-10` (Trixie, Python 3.13) — that box should be the one that works. |
| `Photage.Release.create_admin/2` recovery | `docs/troubleshooting.md`, `docs/configuration.md` | Function exists at `lib/photage/release.ex:183`. Not run through `bin/photage eval` in a container. |
| The import folder works | `README.md`, `docker-compose.yml` | `PHOTAGE_PHOTO_SOURCE=/import` is set and bind-mounted, but the app's default was `/home/photo/sample_photos` — a host path that has never existed in a container. Confirm the import screen actually starts there. |
| `pg_dump`/`pg_restore` recipes | `docs/upgrading.md` | Standard Postgres, not run against this stack. |
| Idle memory is "higher than it should be" | `README.md` | True per `claude/docker-web-image.md`; no number measured. Measure one and put it in the README — "Photage idles at N MB" is the question every self-hoster asks first. |
