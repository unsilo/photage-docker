#!/usr/bin/env bash
#
# Photage installer. Fetches the compose files, generates the secrets, writes a
...
case "$(uname -m)" in
  aarch64|arm64) ;;
  armv7l|armv6l) die "32-bit ARM is not supported. Photage needs a 64-bit OS —
  on a Raspberry Pi that means the 64-bit build of Raspberry Pi OS." ;;
  x86_64|amd64)
    if [[ -z "${ALLOW_EMULATION:-}" ]]; then
      die "Photage ${PHOTAGE_TAG} is published for linux/arm64 only, and this is an
...
say "installing into ${PHOTAGE_DIR}"
mkdir -p "${PHOTAGE_DIR}"
cd "${PHOTAGE_DIR}"
...
mkdir -p scripts
for s in env-detect.sh update-photage download-models.sh upgrade-hailort.sh rollback-hailort.sh; do
...
if [[ -f .env ]]; then
  ...
  for var in PHOTAGE_WAREHOUSE_PATH PHOTAGE_IMPORT_PATH PHOTAGE_MODELS_PATH; do
    val="$(grep -E "^${var}=" .env | head -1 | cut -d= -f2- || true)"
    [[ -z "$val" ]] && continue
    # A value with no slash is a named volume, not a path — leave it to Docker.
    [[ "$val" != */* ]] && continue
    label="${var#PHOTAGE_}"; label="${label%_PATH}"
    ensure_dir "${val/#\~/$HOME}" "$(echo "$label" | tr '[:upper:]' '[:lower:]')"
  done
  # The compose default when PHOTAGE_IMPORT_PATH is unset.
  grep -qE '^PHOTAGE_IMPORT_PATH=' .env || ensure_dir "${PHOTAGE_DIR}/import" "import"
else
  say "generating .env"

  data_root="${PHOTAGE_DATA_ROOT:-}"
  if [[ -z "$data_root" ]] && [[ -r /dev/tty ]]; then
    printf '\n'
    echo "Where should Photage keep your photos?"
    echo
    ...
    echo
    read -r -p "data directory [${HOME}/photage-data]: " ans < /dev/tty || true
    data_root="${ans:-${HOME}/photage-data}"
  fi
  data_root="${data_root:-${HOME}/photage-data}"
  ...
  admin_pass="${PHOTAGE_ADMIN_PASSWORD:-}"
...
PHOTAGE_ADMIN_EMAIL=${admin_email}
PHOTAGE_ADMIN_PASSWORD=${admin_pass}
...
PHOTAGE_IMPORT_PATH=${data_root}/import
...
if [[ "$PORT_EFFECTIVE" == "80" ]] && ss -ltn 2>/dev/null | grep -qE ':80\s'; then
  warn "something is already listening on port 80. Set PHOTAGE_HTTP_PORT (and
       PHOTAGE_IMAGE_URL_BASE) in .env before starting — see .env.example."
  printf '\n'
fi
...
say "installed into ${PHOTAGE_DIR} — nothing started yet"
printf '\n'
...
printf '\n'
echo "  then                 ${url}"
echo "  log in with          the admin email and password in ${PHOTAGE_DIR}/.env"
echo "  photos to import ->  ${import_dir:-${PHOTAGE_DIR}/import}"
echo "  logs             ->  cd ${PHOTAGE_DIR} && docker compose logs -f photage"
echo "  stop             ->  cd ${PHOTAGE_DIR} && docker compose down"
echo "  update later     ->  cd ${PHOTAGE_DIR} && ./scripts/update-photage"
printf '\n'
echo "  update-photage backs up the database, takes the current compose files,"
