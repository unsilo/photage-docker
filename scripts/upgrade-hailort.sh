...
info ""
info "Photage's own HEFs are not at risk — they are on a host directory and are"
...
if [[ -f "${COMPOSE_DIR}/docker-compose.yml" ]] && command -v docker >/dev/null 2>&1; then
  log "Stopping Photage while the driver changes underneath it"
...
info "  ./scripts/env-detect.sh --append"
info ""
...
# This is the only copy of this script — the photage repo's deploy/scripts/ copy
...
if [[ ! -f "${COMPOSE_DIR}/docker-compose.yml" ]]; then
  ...
  info "the models from the photage source tree instead, and re-run the self-test:"
  info ""
  info "  mix photage.download_models --version ${VERSION} --arch hailo10h"
