#!/usr/bin/env bash
#
# Look at this host, work out which of Photage's optional capabilities it can
...
#
#   * ABSENT IS NOT BROKEN. Everything in Photage works with no accelerator, no
...
#     install.sh and update-photage, which both call it.
...
# interpreter. Raspberry Pi OS Trixie and the Photage image are both on Python
...
echo
echo "Photage — host capability detection"
echo "----------------------------------"
echo
...
echo
if [[ ! -x "$PY" ]]; then
  bad "${PY} is not executable — set PHOTAGE_PYTHON to your system interpreter"
else
...
if [[ $HAILO_USABLE -eq 0 ]]; then
  ...
  echo
  echo "That is not necessarily wrong — everything in Photage works without one."
  ...
  # is a fully detected machine, not a failed detection — everything in Photage
  # runs on it — and install.sh and update-photage both call this under `set -e`,
...
# install.sh writes PHOTAGE_MODELS_PATH, so mentioning it again is noise in the
...
#PHOTAGE_MODELS_PATH=${HOME}/photage-data/models"
fi
...
if [[ "$APPEND" == "1" ]]; then
  if [[ ! -f .env ]]; then
    echo "no .env in $(pwd) — run this from your Photage directory" >&2
    exit 1
  fi
